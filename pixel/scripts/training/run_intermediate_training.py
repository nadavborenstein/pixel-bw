...
def get_datasets(model_args, data_args, training_args, rendering_args):
    ...
    train_image_datasets = [
        ...
        for d_name, d_split, d_cache in zip(
            data_args.real_train_dataset_names,
            data_args.real_train_splits,
            data_args.real_dataset_caches,
        )
    ]
    
    test_image_dataset = load_dataset("Nadav/MiniScans", split="test", cache_dir="/scratch/project/dd-22-70/cache/data/miniscans/")
    
    image_dataset_sizes = [ds._info.splits.total_num_examples for ds in train_image_datasets]
    combined_image_size = sum(image_dataset_sizes)
    image_dataset_sampling_probs = [d_size / combined_image_size for d_size in image_dataset_sizes]
    train_image_dataset = interleave_datasets(
        train_image_datasets,
        probabilities=image_dataset_sampling_probs,
        seed=training_args.seed,
    )
    
    train_dataset = HistoricDatasetForPretraining(args=rendering_args, data=train_image_dataset, rng=rng, transform=None)
    test_dataset = HistoricDatasetForPretraining(args=rendering_args, data=test_image_dataset, rng=rng)
...
def main(config_dict: Dict[str, Any] = None):
    ...
    train_dataset, real_dataset_sampling_probs, validation_dataset = get_datasets(
    ...
    if model_args.model_name_or_path:
        logger.info(f"Training from pretrained model {model_args.model_name_or_path}")
