...
class HistoricDatasetForPretraining(Dataset):
    ...
    def get_preprocessing_function(self):
        ...
        categories.sort()
        label_map = defaultdict(lambda: self.args.number_of_labels - 1)  # if in test or train dataset we don't have a class, we can still return a label
    ...
    def resize_image(self, image):
        ...
        if (
            width != self.args.figure_size[0]
        ):  # resize the image if the width is not the same as the specified width, without changing the aspect ratio
            ratio = width / self.args.figure_size[0]
            new_length = int(length / ratio)
            image = image.resize(
                (self.args.figure_size[0], new_length), Image.ANTIALIAS
            )

        if image.size[1] > self.args.figure_size[1]:  # crop the image if it's too long
            image = image.crop((0, 0, image.size[0], self.args.figure_size[1]))

        if self.args.embed_real_image:
            if image.size[1] < self.args.figure_size[0]:
                embedded_image = Image.new(
                    "RGB",
                    (self.args.figure_size[0], self.args.figure_size[1]),
                    (255, 255, 255),
                )
            ...
            if image.size[1] == self.args.figure_size[0]:
                ...
                num_patches = (
                    (self.args.figure_size[0] - image.size[1])
                    // self.args.patch_base_size[0]
                ) * (self.args.figure_size[1] // self.args.patch_base_size[1])
                embedded_image = Image.new(
                    "RGB",
                    (self.args.figure_size[0], self.args.figure_size[1]),
                    (255, 255, 255),
    ...
    def __getitem__(self, item):
        ...
        if not self.labeled:
            _, patch_mask = self.generate_random_mask(self.args.figure_size)
        ...
        #     breakpoint()
        if self.labeled:
            assert label in range(self.args.number_of_labels) or label is None, "label is not in range: {}".format(label)
